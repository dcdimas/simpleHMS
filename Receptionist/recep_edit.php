...
<body>
	...
	<div class="area">
		<h1 class="htitle">Receptionist | Edit Profile</h1>
...
<?php if (isset($_SESSION['wrongcombi'])): ?>
	...
	<?php while ($row = mysqli_fetch_array($results)) { ?>
		...
		<div class="edit">
			...
			<input type="text" name="fullname" value="<?php echo $row['rFull_name']; ?>" required/ >
		</div>
		<div class="edit">
			<label for="bday">Birthday:</label>
			<input type="date" name="bday" value="<?php echo $row['rBirthday']; ?>" required/>
		</div>
		<div class="edit">
			<label for="sched">Schedule:</label>
			<textarea name="sched" class="edit" cols="50" rows="10" required/><?php echo $row['rSchedule']; ?></textarea>
		</div>
		
		<div class="edit">
			<label for="address">Address:</label>
			<input type="text" name="address" value="<?php echo $row['rAddress']; ?>" required/>
		</div>
		<div class="edit">
			<label for="contact">Contact number:</label>
			<input type="text" name="contact" value="<?php echo $row['rContact_no']; ?>" required/>
		</div>
		<div class="edit">
			<label for="email">Email Address:</label>
			<input type="email" name="email" value="<?php echo $row['rEmail_Add']; ?>" >
		</div>
		<div class="edit">
			<label for="username">Username:</label>
			<input type="text" name="username" readonly value="<?php echo $row['rUsername']; ?>" required/>
		</div>
		<div class="edit">
			<label for="pass1">Password:</label>
			<input type="password" name="pass1" value="<?php echo $row['rPassword']; ?>" required/>
		</div>
		<div class="edit">
		...
		<?php if(isset($_POST['submitnurse']))
{
		...
		$Password2 = $_POST['pass2'];
			...
			header('location: recep_edit.php');
			# code...
		} else {
