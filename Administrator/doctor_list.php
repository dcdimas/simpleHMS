...
<body>
	...
	<div id="search">
			...
			$search = $_POST['search'];
			$sql = "SELECT DISTINCT ID, dFull_name, dBirthday, dSpecialization, dSchedule, dAddress, dContact_no, dEmail_Add FROM doctor_register  WHERE dFull_name LIKE '%$search%'";
		}else{
			$sql = "SELECT DISTINCT ID, dFull_name, dBirthday, dSpecialization, dSchedule, dAddress, dContact_no, dEmail_Add FROM doctor_register ";
			$search ="";
		}
			$results = mysqli_query($db, $sql );
		 ?>
			<form method="post" action="doctor_list.php">
			...
			<table class="tbl" width="80%">
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr class="tr">
						...
						<td class="td"><a href="remove_doctor.php" style="text-decoration: none;">Delete</a></td>
					</tr>
				<?php } ?>
				</table>
	</div>
